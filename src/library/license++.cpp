...
EVENT_TYPE acquire_license_client(const char * product,	LicenseLocation licenseLocation, LicenseInfo* license,PcSignature identifier_out) {
	...
	if (licenses.size() > 0) {
		...
		for (auto it = licenses.begin(); it != licenses.end(); it++) {
			if(strncmp(it->client_signature.c_str(),(char*)identifier_out,24)!=0){
				cout<<"client_signature "<<it->client_signature.c_str()<<" and "<<identifier_out;
