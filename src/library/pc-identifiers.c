...
FUNCTION_RETURN generate_ethernet_pc_id(PcIdentifier * identifiers,
	...
	if (identifiers == NULL || *num_identifiers == 0) {
		...
		adapterInfos = (OsAdapterInfo*) malloc(
				adapters * sizeof(OsAdapterInfo) +sizeof(char));
		...
		if (result_adapterInfos == FUNC_RET_BUFFER_TOO_SMALL
			...
			for (j = 0; j < adapters; j++) {
				for (k = 0; k < 6; k++)
					if (use_mac) {
						identifiers[j][k] = adapterInfos[j].mac_address[k];
					} else {
						//use ip
						if (k < 4) {
				...
				//hexDump("identifier:line-125 ",identifiers[j],7);
			}
			result_adapterInfos = (
		...
		free(adapterInfos);
		//hexDump("identifier:line-131 ",identifiers[1],7);
	}
...
FUNCTION_RETURN generate_pc_id(PcIdentifier * identifiers,
	...
	if (result == FUNC_RET_OK && identifiers != NULL) {
		...
		for (i = 0; i < *array_size; i++) {
			...
			identifiers[i][6] = strategy_num;
			//identifiers[i][6] = strategy_num;
			//hexDump("identifiers line:240",identifiers[i],7);
		}
		//fill array if larger
		for (i = *array_size; i < original_array_size; i++) {
			//identifiers[i][0] = STRATEGY_UNKNOWN << 5;
			identifiers[i][6] = STRATEGY_UNKNOWN << 5;
...
FUNCTION_RETURN encode_pc_id(PcIdentifier identifier1, PcIdentifier identifier2,
	...
	sprintf(pc_identifier_out, "%.4s-%.4s-%.4s-%.4s-%.4s", &b64_data[0],
			&b64_data[4], &b64_data[8], &b64_data[12], &b64_data[16]);
...
#endif
	identifier_mac[6] = strategy << 5;//(identifier_mac[6] & 15) | strategy;
	//hexDump("identifiers 1",identifiers[1],7);
	//hexDump("identifier_mac",identifier_mac,7);
	if(memcmp(identifier_mac,identifiers[1],6)!=0){
		printf("identifiers do not match\n");
		//hexDump("identifiers 1",identifiers[1],7);
		//hexDump("identifier_mac",identifier_mac,7);
	}
	result = encode_pc_id(identifiers[0], identifier_mac, identifier_out);
#ifdef __linux__
...
FUNCTION_RETURN decode_pc_id(PcIdentifier identifier1_out,
	...
	sscanf(pc_signature_in, "%4s-%4s-%4s-%4s-%4s", &base64ids[0], &base64ids[4],
			&base64ids[8], &base64ids[12],&base64ids[16]);
...
IDENTIFICATION_STRATEGY strategy_from_pc_id(PcIdentifier identifier) {
	...
	return (IDENTIFICATION_STRATEGY) identifier[6] >> 5;
	//return (IDENTIFICATION_STRATEGY) identifier[6];
}
...
EVENT_TYPE validate_pc_signature(PcSignature str_code) {
	...
	for (i = 0; i < 2; i++) {
		...
		for (j = 0; j < calc_identifiers_size; j++) {
			...
			//hexDump ("print calculated_identifiers", calculated_identifiers[j],7);
			//hexDump ("print user_identifiers", user_identifiers[j],7);
#endif
